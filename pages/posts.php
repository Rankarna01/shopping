...
<body class="bg-white text-gray-800">
    ...
    <div class="container mx-auto px-4">
        <h2 class="text-2xl sm:text-3xl font-bold mb-8 text-center text-gray-800">POST KASIR</h2>

        <!-- Filter Button -->
        <div class="flex flex-wrap gap-3 justify-center mb-8">
            <button onclick="filterCategory('all')" class="bg-green-600 text-white px-5 py-2 rounded-full hover:bg-gray-300 transition text-sm sm:text-base">
                Lihat Semua
            </button>
            <?php foreach ($categories as $category): ?>
                <button onclick="filterCategory('<?= $category; ?>')" class="bg-green-600 text-white px-5 py-2 rounded-full hover:bg-gray-300 transition text-sm sm:text-base">
                    <?= ucfirst($category); ?>
                </button>
            <?php endforeach; ?>
        </div>
        ...
        <!-- Produk -->
        <div id="product-list" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 bg-green-50 p-4 rounded-lg">
            <?php foreach ($products as $product): ?>
                <div class="bg-white p-4 rounded-lg shadow-lg transition duration-300 hover:shadow-xl hover:scale-105 product-item"
                    ...
                    <img src="../uploads/<?= htmlspecialchars($product['image']); ?>" alt="<?= htmlspecialchars($product['name']); ?>"
                        ...
                        onclick="openImageModal('../uploads/<?= htmlspecialchars($product['image']); ?>', '<?= htmlspecialchars($product['name']); ?>')">

                    <h3 class="text-sm font-semibold mt-2"><?= htmlspecialchars($product['name']); ?></h3>
                    <p class="text-xs text-gray-600">Kategori: <?= htmlspecialchars($product['category']); ?></p>
                    <p class="text-xs text-gray-600">Stok: <?= htmlspecialchars($product['stock']); ?></p>
                    <p class="text-sm text-red-600 font-bold mt-2">Rp <?= number_format($product['price']); ?></p>

                    <div class="mt-3 text-left">
                        <button onclick="openModal(<?= $product['id']; ?>, '<?= htmlspecialchars($product['name']); ?>', <?= $product['stock']; ?>)"
        ...
        <!-- Modal Gambar -->
        <div id="imageModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50 p-4">
            <div class="bg-white p-4 sm:p-6 rounded-lg shadow-lg max-w-md w-full relative max-h-screen overflow-y-auto">
    ...
    <div id="modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 transition duration-300 ease-in-out">
        <div class="bg-white p-6 rounded-xl w-11/12 max-w-md shadow-2xl transform transition-all duration-300">
            ...
            <!-- Form -->
            <form id="checkout-form" action="checkout_posts.php" method="POST" class="space-y-4">
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t mt-4">
                    <form action="checkout_posts.php" method="get" class="w-full md:w-auto">
